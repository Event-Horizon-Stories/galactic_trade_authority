defmodule PlanetaryLawTest do
  use ExUnit.Case

  alias Ash.Error.Invalid
  alias PlanetaryLaw.Shipment
  ...
  test "accepts a taxed shipment and records the local adjustments" do
    %{shipment: shipment, applied_rules: applied_rules} = PlanetaryLaw.bootstrap_registry!()
  ...
  test "rejects a shipment banned by a destination planet" do
    ...
    } = PlanetaryLaw.bootstrap_registry!()
  ...
  test "lists the accepted shipments in the chapter registry" do
    %{shipment: shipment} = PlanetaryLaw.bootstrap_registry!()

    [listed_shipment] = PlanetaryLaw.list_shipments!()
