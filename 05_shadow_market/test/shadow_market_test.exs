defmodule ShadowMarketTest do
  ...
  test "official water shipments still keep their local tax path" do
    %{standard_water_shipment: shipment} = ShadowMarket.bootstrap_registry!()
  ...
  test "contracts still permit restricted official shipments" do
    %{permitted_restricted_shipment: shipment} = ShadowMarket.bootstrap_registry!()
  ...
  test "official manifest visibility is still filtered by faction" do
    ...
    } = ShadowMarket.bootstrap_registry!()

    guild_manifests =
      ShadowMarket.visible_manifests!(guild_actor)
      |> Enum.map(& &1.manifest_number)
      |> Enum.sort()

    syndicate_manifests =
      ShadowMarket.visible_manifests!(syndicate_actor)
      |> Enum.map(& &1.manifest_number)
      |> Enum.sort()
