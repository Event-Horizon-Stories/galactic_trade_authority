defmodule InvestigationTest do
  use ExUnit.Case

  alias Ash.Error.Forbidden
  alias Ash.Error.Invalid
  alias Investigation.Shipment
  alias Investigation.ShadowReport
  ...
  test "official water shipments still keep their local tax path" do
    %{standard_water_shipment: shipment} = Investigation.bootstrap_registry!()
  ...
  test "contracts still permit restricted official shipments" do
    %{permitted_restricted_shipment: shipment} = Investigation.bootstrap_registry!()
  ...
  test "official manifest visibility is still filtered by faction" do
    ...
    } = Investigation.bootstrap_registry!()

    guild_manifests =
      Investigation.visible_manifests!(guild_actor)
      |> Enum.map(& &1.manifest_number)
      |> Enum.sort()

    syndicate_manifests =
      Investigation.visible_manifests!(syndicate_actor)
      |> Enum.map(& &1.manifest_number)
      |> Enum.sort()
  ...
  test "audit trail records who registered, reviewed, and later flagged a manifest" do
    %{investigated_shipment: shipment} = Investigation.bootstrap_registry!()
    ...
    assert [
           ...
           ] = Investigation.audit_trail_for_manifest!(shipment.manifest_number)
  ...
  test "case files explain who approved a slipped shipment and who flagged it later" do
    %{case_file: case_file, investigated_shipment: shipment} = Investigation.bootstrap_registry!()
  ...
  test "unmatched evidence still becomes a traceable audit event" do
    %{unmatched_report: report} = Investigation.bootstrap_registry!()

    assert report.ledger_status == :unmatched

    assert Investigation.audit_trail_for_manifest!(report.reported_manifest) == [
             %{
               audit_code: "AR-6006",
               event_type: :shadow_report_recorded,
               finding: :unmatched,
